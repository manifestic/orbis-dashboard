      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <section className="min-w-0 flex-1 px-4 py-5 sm:px-7 lg:px-10 lg:py-8">
          <header className="mx-auto max-w-[1380px]">
            ...
            {activeSection === "overview" ? <PortalOverview isLive={isLive} unreadCount={unreadCount} firstAppointment={firstAppointment} openTasks={visibleTasks.length} onGoToSection={goToSection} inboxHref={ghl("/conversations/conversations/?category=team-inbox&tab=unread")} calendarHref={ghl("/calendars/view")} opportunitiesHref={ghl("/opportunities/list")} plannerHref={ghl("/marketing/social-planner")} /> : <DashboardSectionView section={activeSection} conversations={visibleConversations} appointments={visibleAppointments} tasks={visibleTasks} live={isLive} unreadCount={unreadCount} onSelectConversation={setSelectedConversation} onOpenInbox={() => setShowAllMessages(true)} inboxHref={ghl("/conversations/conversations/?category=team-inbox&tab=unread")} calendarHref={ghl("/calendars/view")} opportunitiesHref={ghl("/opportunities/list")} plannerHref={ghl("/marketing/social-planner")} />}
...
function PortalOverview({ isLive, unreadCount, firstAppointment, openTasks, onGoToSection, inboxHref, calendarHref, opportunitiesHref, plannerHref }: { isLive: boolean; unreadCount: number; firstAppointment?: LiveAppointment; openTasks: number; onGoToSection: (section: DashboardSection) => void; inboxHref?: string; calendarHref?: string; opportunitiesHref?: string; plannerHref?: string }) {
...
function DashboardSectionView({ section, conversations: visibleConversations, appointments: visibleAppointments, tasks: visibleTasks, live, unreadCount, onSelectConversation, onOpenInbox, inboxHref, calendarHref, opportunitiesHref, plannerHref }: { section: DashboardSection; conversations: LiveConversation[]; appointments: LiveAppointment[]; tasks: LiveTask[]; live: boolean; unreadCount: number; onSelectConversation: (name: string) => void; onOpenInbox: () => void; inboxHref?: string; calendarHref?: string; opportunitiesHref?: string; plannerHref?: string }) {
...
function ContentReviewCard({ plannerHref, socialHref, calendarHref }: { plannerHref?: string; socialHref?: string; calendarHref?: string }) {

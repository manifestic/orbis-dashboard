      ...
      <div className="relative z-10 flex min-h-screen">
        <aside className="hidden w-[238px] shrink-0 border-r border-white/[0.07] bg-[#0b0f1a]/90 px-4 py-5 lg:flex lg:flex-col">
          ...
          <nav className="space-y-1 text-sm">
            ...
            <SideNavItem icon={Inbox} label="Inbox" badge="2" href={ghl("/conversations/conversations/?category=team-inbox&tab=unread")} />
            ...
            <SideNavItem icon={Globe2} label="Websites" href="https://yourbesthealthquote.vercel.app/" newTab />
        ...
        <section className="min-w-0 flex-1 px-4 py-5 sm:px-7 lg:px-10 lg:py-8">
          <header className="mx-auto max-w-[1380px]">
            ...
            <section className="mt-8 grid items-start gap-5 xl:grid-cols-[1.45fr_0.8fr]"><InboxPreview conversations={visibleConversations} live={isLive} unreadCount={unreadCount} selectedConversation={selectedConversation} onSelect={setSelectedConversation} onOpenFull={() => setShowAllMessages(true)} inboxHref={ghl("/conversations/conversations/?category=team-inbox&tab=unread")} /><CalendarPreview appointments={visibleAppointments} live={isLive} calendarHref={ghl("/calendars/view")} /></section>
            ...
            <section className="mt-10 pb-10"><SectionHeading eyebrow="Websites & intelligence" title="The assets Manifestic is building with you" detail="A single place to open your sites, review performance, and find the latest intelligence." /><div className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]"><WebsitesCard sitesHref="https://yourbesthealthquote.vercel.app/" /><ReportsCard reportsHref={ghl("/reporting/reports")} /></div></section>
...
function SideNavItem({ icon: Icon, label, active, badge, href, newTab }: { icon: IconType; label: string; active?: boolean; badge?: string; href?: string; newTab?: boolean }) { const className = `flex items-center gap-3 rounded-xl px-3 py-2.5 ${active ? "bg-cyan-300/10 text-cyan-100" : "text-slate-500 hover:bg-white/[0.04] hover:text-slate-200"}`; return href ? <a className={className} href={href} target={newTab ? "_blank" : "_top"} rel={newTab ? "noopener noreferrer" : "noreferrer"}><Icon className="h-4 w-4" /><span className="flex-1">{label}</span>{badge && <span className="rounded-full bg-violet-400/15 px-1.5 py-0.5 text-[10px] text-violet-200">{badge}</span>}</a> : <div className={className}><Icon className="h-4 w-4" /><span className="flex-1">{label}</span>{badge && <span className="rounded-full bg-violet-400/15 px-1.5 py-0.5 text-[10px] text-violet-200">{badge}</span>}</div>; }
